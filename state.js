...
export const PNAMES = ['Guarda-Redes','Lateral Dir.','Central Esq.','Central Dir.','Lateral Esq.','M??dio Int.','Trinco','Meia','Ext. Esq.','Avan??ado','Ext. Dir.'];
export const PNAMES_OPP = ['GR Adv.','Lat.Dir. Adv.','Def.Esq. Adv.','Def.Dir. Adv.','Lat.Esq. Adv.','Med. Adv.','Trinco Adv.','Meia Adv.','Ext.Esq. Adv.','Avan??ado Adv.','Ext.Dir. Adv.'];
...
export const SHAPE_NAMES = { polygon:'Zona', zone:'??rea', arrow:'Mov.', linked:'Liga????o', ruler:'Medida', curve:'Curva', spotlight:'Foco' };
export const HINTS = {
  polygon: 'Pol??gono ??? clique para pontos ?? duplo-clique para fechar',
  ...
  ruler:   'R??gua ??? arraste ou clique p/ ponto A ?? duplo clique p/ ponto B',
  curve:   'Seta Curva ??? clique in??cio, clique meio(curva), duplo clique fim',
...
export const BPZ = {
  ...
  pen: { pts:[{x:30,y:13},{x:38,y:13},{x:38,y:19},{x:30,y:19}],fill:'rgba(232,85,85,.15)',  stroke:'rgba(232,85,85,.7)',   lbl:'Pen??lti',     ball:{x:34,y:16} }
};
export const PITCH = {
...
export function restoreSession(session) {
  if (!session || typeof session !== 'object') return 'Sess??o inv??lida';
  try {
