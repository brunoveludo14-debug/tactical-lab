...
import {
  ...
  renderNotesList, renderSaveList
} from './render.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  // Auth removed — initialize directly without login
...
function switchView(v, btn) {
  ...
  setTimeout(() => {
    ...
    toolbar.classList.toggle('tb-hidden', v === 'notes' || v === 'calendar');
...
function bindToolbar() {
  ...
  // Tool bindings
  document.getElementById('t-arrow')?.addEventListener('click', function() { setMode('arrow', this); });
  document.getElementById('t-dashed')?.addEventListener('click', function() { setMode('dashed-arrow', this); });
  document.getElementById('t-pencil')?.addEventListener('click', function() { setMode('pencil', this); });
  document.getElementById('t-text')?.addEventListener('click', function() { setMode('text', this); });
  document.getElementById('t-eraser')?.addEventListener('click', function() { setMode('eraser', this); });
  ...
  document.querySelectorAll('.thick-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      this.classList.add('active');
      _drawThick = parseFloat(this.dataset.thick);
    });
  });
  ...
  ['t','b','p'].forEach(w => {
    ...
    if (mode !== 'none') {
      ...
      if (mode === 'eraser') {
        ...
        pitch.classList.add('eraser-mode');
      }
      hintEl?.classList.remove('hide');
      if (hintTxt) hintTxt.textContent = HINTS[mode] || '';
    } else {
      pitch.classList.remove('xhair');
      pitch.classList.remove('eraser-mode');
  ...
  if (modeBar) {
    ...
    if (modeBarTxt) {
      modeBarTxt.textContent = {
        polygon:      'Polígono — clica pontos no campo · duplo-clique para fechar',
        zone:         'Zona — clica 2 pontos para definir a área',
        arrow:        'Seta — clica ponto A, depois ponto B',
        linked:       'Linha — clica nos jogadores · duplo-clique para fechar',
        ruler:        'Régua — clica ponto A, depois ponto B para medir',
        curve:        'Curva — clica início, controlo e fim (3 pontos)',
        spotlight:    'Foco — clica centro, depois limite do círculo',
        eraser:       'Borracha — clica numa forma para a apagar',
      }[mode] || '';
    }
  }
}
  ...
  ['t','b','p'].forEach(w => {
    ...
    el.addEventListener('pointerdown', e => {
      if(State.mode === 'pencil') {
        isDrawing = true;
        const pct = getPct(e, id);
        const pt = pctVb(pct.x, pct.y, PITCH[w].vw, PITCH[w].vh);
    ...
    el.addEventListener('pointermove', e => {
      if(isDrawing && State.mode === 'pencil') {
        const pct = getPct(e, id);
        const pt = pctVb(pct.x, pct.y, PITCH[w].vw, PITCH[w].vh);
        const drawArr = w === 't' ? State.tDraw : w === 'b' ? State.bDraw : State.pDraw;
        drawArr.push(pt);
    ...
    el.addEventListener('pointerup', e => {
      if(isDrawing && State.mode === 'pencil') {
        ...
        if(drawArr.length > 1) {
          pushHistory();
          commitShape(w, 'pencil', [...drawArr]);
        }
        if(w === 't') State.tDraw = []; else if(w === 'b') State.bDraw = []; else State.pDraw = [];
...
function attachPlayerEvents(el, playerId, w, pitchId, vw, vh) {
  ...
  el.addEventListener('click', e => {
    ...
    if (State.mode === 'linked') {
      ...
      renderDrawPreview(w, State.mode, drawArr, _drawColor, _drawThick);
      return;
    }
 ...
 */
export function rebuildAllFromState() {
  // Tactic pitch — team
  clearPlayerEls('pitch', ['pl-opp','pl-opp-gk']);
  State.players.forEach((pl, i) => {
    const el = createPlayerEl(`tp${pl.id}`, pl.n, pl.isGk ? 'gk' : 'f', pl.name, pl.x, pl.y, i, 68, 105);
