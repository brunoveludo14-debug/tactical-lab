...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hasAutosave()) {
    ...
    if (!err) {
      rebuildAllFromState();
      showToast('Sess??o anterior restaurada');
    }
  } else {
...
function bindToolbar() {
  ...
  document.getElementById('btn-clear-box-pl')?.addEventListener('click', async () => {
    if (State.bPlayers.length) {
      const ok = await customConfirm('Remover jogadores', 'Remover todos os jogadores da grande ??rea?');
      if (!ok) return;
    }
    clearBoxPlayers();
  });
  ...
  if (modeBar) {
    ...
    if (modeBarTxt) {
      modeBarTxt.textContent = {
        polygon:      'Modo Pol??gono ??? clique para pontos ?? duplo-clique para fechar',
        ...
        ruler:        'Modo R??gua ??? clique ponto A, depois clique ponto B para medir',
        curve:        'Modo Curva ??? clique in??cio, ponto curva, duplo-clique fim',
...
function renderTimeline() {
  ...
  frames.forEach((kf, idx) => {
    ...
    frame.addEventListener('drop', e => { e.preventDefault(); frame.classList.remove('kf-drop-target');
      ...
      renderTimeline(); showToast(`Frame movido para posi????o ${idx+1}`); });
...
function openSaveModal() {
  document.getElementById('modal-title-txt').textContent = 'Guardar Sess??o';
...
function openLoadModal() {
  document.getElementById('modal-title-txt').textContent = 'Carregar Sess??o';
...
function refreshSaveList() {
  renderSaveList(getSaves(), (id) => {
    ...
    if (!confirm('Eliminar esta sess??o?')) return;
    deleteSession(id);
    refreshSaveList();
  });
}
...
function renderLibrary(filter) {
  ...
  grid.querySelectorAll('[data-lib-del]').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
      const ok = await customConfirm('Eliminar jogada', 'Esta jogada ser?? eliminada permanentemente.');
      if (!ok) return;
...
function saveCurrentPlay() {
  ...
  if (!name) { nameIn?.focus(); showToast('D?? um nome ?? jogada'); return; }
...
function renderCustomFmtList() {
  ...
  if (!fmts.length) {
    html += `<div class="cfmt-empty">Nenhuma forma????o personalizada</div>`;
  } else {
    html += fmts.map((f, i) => `
  ...
  html += `<button class="cfmt-save-btn" id="cfmt-save-now">??? Guardar forma????o atual</button>`;
  ...
  list.querySelectorAll('.cfmt-item').forEach(item => {
    item.addEventListener('click', e => {
      ...
      scheduleAutosave();
      list.classList.remove('open');
      showToast(`Forma????o "${f.name}" carregada`);
    });
  });
  ...
  list.querySelectorAll('[data-cfmt-del]').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      const f   = State.customFmts[idx];
      const ok  = await customConfirm('Eliminar forma????o', `Eliminar "${f?.name}"?`);
      if (!ok) return;
    ...
    input.onchange = () => {
      ...
      reader.onload = ev => {
        try {
          ...
          if (data._type !== 'tl-custom-fmts' || !Array.isArray(data.fmts)) throw new Error('Formato inv??lido');
          ...
          localStorage.setItem('tl_custom_fmts', JSON.stringify(State.customFmts));
          renderCustomFmtList();
          showToast(`${imported} forma????o(??es) importada(s)`);
        } catch (err) {
...
async function exportMP4() {
  if (State.keyframes.length < 2) { showToast('Precisa de 2+ fotogramas para exportar v??deo'); return; }

  const pitchEl = document.getElementById('pitch-wrap') || document.getElementById('pitch');
  if (!pitchEl) { showToast('Elemento do campo n??o encontrado'); return; }

  showToast('A preparar grava????o???');
  ...
  if (!mimeType) {
    ...
    try {
      const ok = confirm('O seu browser n??o suporta grava????o directa. Vamos usar a captura de ecr??.');
      if (!ok) return;
      ...
      recorder.onstop = () => {
        ...
        URL.revokeObjectURL(url); showToast('V??deo exportado!'); stream.getTracks().forEach(t => t.stop());
      };
      recorder.start();
      showToast('A gravar??? Anima????o vai come??ar');
      setTimeout(async () => { await playKF(); setTimeout(() => recorder.stop(), 800); }, 1500);
    } catch (e) { showToast('Grava????o cancelada'); }
    return;
  }
  ...
  recorder.onstop = () => {
    ...
    a.remove();
    URL.revokeObjectURL(url);
    showToast('???? V??deo exportado com sucesso!');
  ...
  if (m) {
    try {
      ...
      if (data?.session) {
        ...
        showToast('??? T??tica partilhada carregada!');
