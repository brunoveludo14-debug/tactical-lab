...
export function renderDrawPreview(w, mode, points) {
  const svgId = w === 't' ? 'p-draw' : w === 'b' ? 'b-draw' : 'pb-draw';
  const svg   = document.getElementById(svgId);
  if (!svg) return;
  svg.innerHTML = '';
  ...
  const c = SHAPE_STYLES[mode] || SHAPE_STYLES.polygon;
  ...
  if (mode === 'arrow' || mode === 'linked' || mode === 'ruler') {
    if (rPts.length >= 2) {
      ...
      if (mode === 'ruler') {
        const [p1, p2] = rPts;
        sa(l, { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, stroke: c.stroke, 'stroke-width': '0.5', 'stroke-dasharray': '0.5 0.5' });
      } else {
        l.setAttribute('points', rPts.map(p => `${p.x},${p.y}`).join(' '));
        sa(l, { fill: 'none', stroke: c.stroke, 'stroke-width': '0.6', 'stroke-dasharray': mode === 'linked'?'0.8 1.2':'', 'stroke-linecap': 'round' });
      }
      svg.appendChild(l);
    }
    rPts.forEach((p, i) => {
      const ci = ns('circle');
      sa(ci, { cx: p.x, cy: p.y, r: i ? '0.7' : '1', fill: c.stroke });
      svg.appendChild(ci);
    });
  } else if (mode === 'curve') {
    if (rPts.length >= 2) {
      ...
      path.setAttribute('d', d);
      sa(path, { fill: 'none', stroke: c.stroke, 'stroke-width': '0.6', 'stroke-linecap': 'round' });
      svg.appendChild(path);
    }
    rPts.forEach((p, i) => {
      const ci = ns('circle');
      sa(ci, { cx: p.x, cy: p.y, r: '0.7', fill: c.stroke });
      svg.appendChild(ci);
    });
    ...
    if (rPts.length >= 2) {
      ...
      const ci = ns('circle');
      sa(ci, { cx: center.x, cy: center.y, r: radius, fill: 'none', stroke: 'rgba(255,255,255,0.8)', 'stroke-width': '0.4', 'stroke-dasharray': '1 1' });
      svg.appendChild(ci);
    }
    rPts.forEach((p, i) => {
      const ci = ns('circle');
      sa(ci, { cx: p.x, cy: p.y, r: '0.7', fill: 'rgba(255,255,255,0.8)' });
      svg.appendChild(ci);
    });
  } else {
    if (rPts.length > 0) {
      ...
      sa(poly, { fill: c.fill, stroke: c.stroke, 'stroke-width': '0.4', 'stroke-dasharray': '1.2 0.6' });
...
function addLabel(text, svgX, svgY, color, w, pitchId, vw, vh, shapeId = null) {
  const pitch = document.getElementById(pitchId);
  if (!pitch) return;
...
export function renderSaveList(saves, onLoad, onDelete) {
  ...
  if (!saves.length) {
    el.innerHTML = '<div class="modal-empty">Nenhuma sess??o guardada ainda.</div>';
    return;
  }
