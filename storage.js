...
export function saveSession(name) {
  const entry = {
    id:   Date.now(),
    name: name.trim() || `Sess??o ${new Date().toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}`,
...
export function loadSession(id) {
  ...
  if (!save) return 'Sess??o n??o encontrada';
...
export function importSessionFromJSON(jsonStr) {
  ...
  try {
    ...
    return 'Ficheiro JSON inv??lido';
  }
  if (!parsed || typeof parsed !== 'object') return 'Formato n??o reconhecido';
